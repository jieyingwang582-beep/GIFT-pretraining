...
###############################################################################
# GIFT 300M run script
#
# This script corresponds to the GIFT geometry-informed FP8 communication
# method for the 300M model configuration.
#
...
CHECKPOINT_PATH="${CHECKPOINT_PATH:-${REPO_DIR}/outputs/gift_300m_checkpoints}"
TENSORBOARD_LOGS_PATH="${TENSORBOARD_LOGS_PATH:-${REPO_DIR}/outputs/tensorboard/gift_300m}"
...
srun \
  ...
  bash -lc "
    ...
    NODE_RANK=\$SLURM_NODEID
    echo \"LAUNCH host=\$(hostname) SLURM_NODEID=\$SLURM_NODEID JOBID=\$SLURM_JOB_ID NODE_RANK=\$NODE_RANK\"
