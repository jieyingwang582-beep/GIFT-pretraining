...
class BaselineFP8Manager:
    def __init__(
        ...
        self._num_layers_for_stats = 0
        for module_name in self._module_name_map.values():
            m = self._fc_pattern.match(module_name)
            if m is not None:
                layer_id = int(m.group(1))
                self._num_layers_for_stats = max(self._num_layers_for_stats, layer_id + 1)

        if self._num_layers_for_stats == 0:
            self._num_layers_for_stats = 32
    ...
    def _empty_step_stats(self):
        stats = {}
        for i in range(self._num_layers_for_stats):
