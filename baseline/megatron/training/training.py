...
def post_training_step_callbacks(
    ...
    if (
        args.profile
        and _should_stop_profile(args, iteration)
...
def _get_profile_window_length(args) -> int:
    """Return the number of training steps covered by one profiling window."""
    return max(args.profile_step_end - args.profile_step_start + 1, 1)


def _should_start_profile(args, next_iteration: int) -> bool:
    """Return True when profiling should start before running `next_iteration`."""
    if next_iteration < args.profile_step_start:
        return False

    if args.profile_step_interval <= 0:
        return next_iteration == args.profile_step_start

    offset = next_iteration - args.profile_step_start
    return offset >= 0 and offset % args.profile_step_interval == 0


def _should_stop_profile(args, completed_iteration: int) -> bool:
    """Return True when profiling should stop after running `completed_iteration`."""
    if completed_iteration < args.profile_step_start:
        return False

    first_window_end = args.profile_step_start + _get_profile_window_length(args) - 1
    if args.profile_step_interval <= 0:
        return completed_iteration == first_window_end

    offset = completed_iteration - first_window_end
    return offset >= 0 and offset % args.profile_step_interval == 0
...
def train(
    ...
    while iteration < args.train_iters:
        if args.profile and torch.distributed.get_rank() in args.profile_ranks:
            if args.use_pytorch_profiler:
                prof.step()
            elif _should_start_profile(args, iteration + 1):
